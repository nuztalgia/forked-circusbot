...
export function registerEventCreator(client: Client) {
        ...
        switch (event.step) {
            ...
            case 'date':
                if (!message.content.match(/^[0-9]{1,2}-(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)-202[2-9]$/)) {
                    sendError(message.channel, 'Invalid Date Format', 'Invalid date or date format was given, please try again');
                    message.react('👎');
                    return;
                }
            ...
            case 'tank_limit':
                if (!message.content.match(/^[0-9]+$/i)) {
                    sendError(message.channel, 'Invalid Format', 'Invalid format, the entered value **must** be a number');
                    message.react('👎');
                    return;
                }
