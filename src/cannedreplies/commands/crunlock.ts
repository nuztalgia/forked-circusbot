...
bot.registerCommand('crunlock', [], message => {
    ...
    if (!name) {
        bot.replyTo(message, EMBED_ERROR_COLOR, makeError('Please specify the name of the canned reply to unlock'));
        return;
    ...
    saveCannedReplies();
    bot.replyTo(message, EMBED_SUCCESS_COLOR, `✅ =${name} has now been unlocked and can only be edited from any channel`);
});
